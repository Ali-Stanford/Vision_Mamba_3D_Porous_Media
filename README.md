...
Vision Mamba has recently received attention as an alternative to Vision Transformers (ViTs) for image classification. The network size of Vision Mamba scales linearly with input image resolution, whereas ViTs scale quadratically, a feature that improves computational and memory efficiency. Both Vision Mamba and ViTs are typically trained on images of a fixed size yet can make predictions on images with different resolutions at inference time, a capability not inherent to traditional convolutional neural networks (CNNs), and thus they can be more memory efficient. Moreover, Vision Mamba requires a significantly smaller number of trainable parameters than CNNs. Because of these features, we introduce, for the first time, a neural network that uses Vision Mamba as its backbone for predicting the permeability of three-dimensional porous media. We compare the performance of Vision Mamba with CNN models across multiple aspects of permeability prediction and perform an ablation study to assess the effects of its components on accuracy. We train Vision Mamba at a fixed size and then evaluate its generalizability by asking it to predict the permeability of porous media at larger and smaller sizes. We make the source code publicly available to facilitate reproducibility and to enable other researchers to build on and extend this work. We believe the proposed framework has the potential to be integrated into large vision models in which Vision Mamba is used instead of ViTs.
