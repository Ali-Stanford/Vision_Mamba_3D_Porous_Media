...
class PatchEmbed3D(nn.Module):
    ...
    def forward(self, x):
        ...
        return x.permute(0, 2, 1)             # [B, seq_len, C]
...
class MambaBlock(nn.Module):
    ...
    def forward(self, x):
        res = self.norm1(x)
        y, _ = self.s4d(res)    # [B, seq_len, dim]
        x = x + y
        res = self.norm2(x)
        x = x + self.ff(res)
        return x
...
def main():
    ...
    # stack along the sample dimension
    cubes = np.concatenate(all_cubes, axis=0)   
    perms = np.concatenate(all_perms, axis=0) 
    cubes = torch.from_numpy(cubes).float().unsqueeze(1) 
    perms = torch.from_numpy(perms).float()  
  
    print("data numer is ", len(perms))

    all_cubes = []
    all_perms = []
             
    ds = CubeDataset(cubes, perms)
    N = len(ds)
    ...
    y_train = torch.stack([y for _,y in train_ds])
    t_min, t_max = y_train.min().item(), y_train.max().item()

    train_loader = DataLoader(train_ds, batch_size=16, shuffle=True, pin_memory=True)
    val_loader   = DataLoader(val_ds,   batch_size=16,  pin_memory=True)
    test_loader  = DataLoader(test_ds,  batch_size=16,  pin_memory=True)
